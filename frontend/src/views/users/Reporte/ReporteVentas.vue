...
const props = defineProps<{
  ...
  fechaInicio: Date
  fechaFin: Date
  modo: TipoReporteUsuario
}>()

// Mostrar encabezado del rango de fechas
const mostrarRangoFechas = () => {
  if (props.modo === 'DIARIO') return format(props.fechaInicio, 'yyyy-MM-dd')
  if (props.modo === 'MENSUAL')
    return `Mes de ${format(props.fechaInicio, 'MMMM yyyy')}`
  if (props.modo === 'RANGO')
    return `${format(props.fechaInicio, 'yyyy-MM-dd')} al ${format(props.fechaFin, 'yyyy-MM-dd')}`
  if (props.modo === 'TOTAL')
    return `Desde inicio hasta ${format(props.fechaFin, 'yyyy-MM-dd')}`
  return ''
}
...
const totalGeneral = computed(() =>
  props.ventas.reduce((acc, v) => acc + recibidoVenta(v), 0).toFixed(2)
)
</script>
