<template>
  <v-card class="pa-4" elevation="2">
    ...
    <v-card-actions class="d-flex justify-space-evenly">
      ...
      <v-btn variant="outlined" color="green" @click="actualizarEvento">
        Aceptar
      </v-btn>
    </v-card-actions>
  </v-card>
...
import { AmbienteInterface, EditarEventoInterface } from '@/interfaces'
import { getAmbientes, getEventoById, updateEvento } from '@/services'
...
const formEvento = reactive<EditarEventoInterface>({
  id: props.id || null,
  nombre: '',
  descripcion: '',
  tipo: 'DIARIO',
...
const obtenerEvento = async () => {
  ...
  try {
    const evento = await getEventoById(props.id)
    Object.assign(formEvento, evento)
    formEvento.ambientes = evento.ambientes.map(a => a.id!)
  } catch (error: any) {
    notify.error(error.message)
  }
}
...
const actualizarEvento = async () => {
  try {
    await updateEvento(props.id!, formEvento)
    notify.success('Evento actualizado exitosamente')
    close()
  } catch (error: any) {
    notify.error(error.message)
  }
