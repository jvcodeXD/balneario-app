<template>
  <v-card>
    <v-card-title> Crear ambiente </v-card-title>
    <v-card-text>
      <v-row>
        <!-- Nombre ambiente -->
        <v-col cols="12" md="7">
    ...
    <v-card-actions>
      ...
      <v-btn color="success" variant="outlined" @click="crearAmbiente">
        Aceptar
      </v-btn>
    </v-card-actions>
  </v-card>
...
import { createAmbiente, getPrecios } from '@/services'
...
const crearAmbiente = async () => {
  try {
    ...
    notify.success('Ambiente creado correctamente')
    close()
  } catch (error: any) {
